...
import { littleGhostsMarketContractAddress } from '../../../ContractAddresses';
import littleGhostsNftMarketplaceAbi from '.';
import { useWeb3React } from '@web3-react/core';

const web3 = new Web3(activeNode);
const multicall = new Multicall({ web3Instance: web3, tryAggregate: true });

const LittleGhostsNftMarketplaceContract = new web3.eth.Contract(
	littleGhostsNftMarketplaceAbi as AbiItem[],
...
export const useMintCount = (account: string) => {
	...
	const getMintCount = useCallback(async () => {
		...
		const mintCount = await LittleGhostsNftMarketplaceContract.methods
...
export const useReflectionBalance = (tokenIds: Array<number>) => {
		...
		return [
			{
				...
				abi: littleGhostsNftMarketplaceAbi,
	...
	}, [tokenIds, littleGhostsMarketContractCallContext]);
...
export const useMintBalance = ({
	...
	const getMintBalance = useCallback(async () => {
		...
		const mintBalance = await LittleGhostsNftMarketplaceContract.methods
...
export const useClaimRewards = (callback: (receipt) => void) => {
	...
	return useCallback(() => {
		const encoded = LittleGhostsNftMarketplaceContract.methods
			.claimRewards()
			.encodeABI();
		library
			?.getSigner()
			.sendTransaction({
				from: account || '',
				to: littleGhostsMarketContractAddress,
				value: 0,
				data: encoded,
			})
			.then((res) => {
