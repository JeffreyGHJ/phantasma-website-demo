...
export const useApproveClaimLootboxReward = () => {
	...
	return useCallback(
		({
			...
			const encoded = LootboxContract.methods
				.setApprovalForAll(lootboxContractAddress, true)
				.encodeABI();
			library
				?.getSigner()
				.sendTransaction({
				...
				.then((res) => {
					handleWeb3Reponse({
						waitingMessage: `Waiting for confirmations to approve claiming Lootbox rewards`,
						successMessage: `Successfully approved! Now you may claim Lootbox rewards`,
						res,
						callback,
					});
				})
