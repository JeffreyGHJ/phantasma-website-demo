...
export const useClaimLootboxReward = () => {
	...
	return useCallback(
		({
			...
			library
				...
				.then((res) => {
					handleWeb3Reponse({
						waitingMessage: `Waiting for confirmations to claim Lootbox #${lootboxes.join(
							', '
						)} rewards`,
						successMessage: `Successfully claimed Lootbox #${lootboxes.join(
							', '
						)} rewards`,
						res,
						callback,
