...
import pancakeLittleGhostsRewardsABI from '../constants/abis/PancakeLittleGhostsRewardsABI';
import { pancakeNFTRewardContractAddress } from '../constants/ContractAddresses';

const web3 = new Web3(activeNode);
const multicall = new Multicall({ web3Instance: web3, tryAggregate: true });

const useClaimableBnbRewards = (
	account: string,
	...
	const pancakeswapRewardsContractCallContext = useMemo(() => {
		const context = [
			{
				reference: 'pcsRewardContract',
				contractAddress: pancakeNFTRewardContractAddress,
				abi: pancakeLittleGhostsRewardsABI,
				calls: [] as Array<{
					reference: string;
					methodName: string;
	...
	useEffect(() => {
		...
		const PancakeNFTRewardContract = new web3.eth.Contract(
			pancakeLittleGhostsRewardsABI as AbiItem[],
		...
		const promises = [
			multicall
				.call(pancakeswapRewardsContractCallContext)
				.then((response) => {
					let totalBalance = 0;
					if (response.results.pcsRewardContract) {
						response.results.pcsRewardContract.callsReturnContext.forEach(
							(ctx) => {
			...
			PancakeNFTRewardContract.methods
	...
	}, [pancakeswapRewardsContractCallContext, account, mintCount]);

	return rewards;
};

export default useClaimableBnbRewards;
