...
function getQueryStringFromStates({
    ...
    const baseURI = "/marketplace/profile/vault/supplies";
...
const InventorySupply = () => {
    ...
    const handleOpenFounderLootbox = useCallback(
        ...
        [openLootbox, updateOverlay, refreshItems]
    );
    ...
    }, [isLootboxApprovedToClaim, approveLootboxForClaim]);
    ...
    const handleClaimFounderLootbox = useCallback(
        (_item?: any) => {
            ...
            claimLootboxRewards({
                lootboxes: [_item ? _item.token_id : selectedItem.token_id],
                callback: (receipt) => {
                    refreshItems();
                    setIsSpinBoxOpen(false);
                },
        ...
        [selectedItem, claimLootboxRewards, refreshItems]
    );

    const lootboxActions = useCallback(
        (_item: any) => {
            if (
                supplyType !== supplyTypes.FOUNDERS_LOOTBOXES ||
                _item.name !== "Founder's Lootboxes"
            ) {
                return undefined;
            }
        ...
        [supplyType, isLootboxApprovedToClaim]
    );
    ...
    }, [account, checkIfLootboxIsApprovedForClaim]);
