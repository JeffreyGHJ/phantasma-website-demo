...
function getQueryStringFromStates({
    ...
    const baseURI = "/marketplace/profile/vault/pets";
...
const InventoryPets = () => {
    ...
    useEffect(() => {
        ...
        const fetchPetsForAccount = (currentAccount) => {
            const params = {
                limit: petsPerPage,
        ...
        Promise.all(allAccounts.map(fetchPetsForAccount))
            .then((responses) => {
                responses.forEach((response) => {
                    // @ts-ignore
                    allPets = [...allPets, ...response.data];
                ...
                if (mounted) {
                    setPets(allPets);
                    setItemsCount(filteredSOLAssets.length + totalItemsCount);
                    ...
                    <>
                        <div className="left">
                            <Title
                                ...
                                //petType={petCollectionNameByType[petType]}
                                petType={"Pets"}
                            />
                            <SaleTypeDropdown
                                saleType={saleType}
