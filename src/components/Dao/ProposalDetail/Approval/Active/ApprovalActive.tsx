...
const ApprovalActive = ({
	...
	const handleSelectedChoicesChange = (index) => {
		const choice = index + 1;
		if (selectedChoices.includes(choice)) {
			setSelectedChoices(selectedChoices.filter((x) => x !== choice));
		} else {
...
const SingleChoiceResultBox = ({
	...
	votesWithScores.forEach((_voteWithScore) => {
		if (isArray(_voteWithScore.choice)) {
			_voteWithScore.choice.forEach((_choice) => {
				choiceScoresObj[_choice] = {
					...choiceScoresObj[_choice],
					score:
						choiceScoresObj[_choice].score +
