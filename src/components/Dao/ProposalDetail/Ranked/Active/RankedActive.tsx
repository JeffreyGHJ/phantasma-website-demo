...
import { isWholeNumber, rank } from '../../../../../utils/NumberUtil';
...
const RankedActive = ({
	...
	const [selectedChoices, setSelectedChoices] = useState([] as Array<number>);
	...
	const addToSelectedChoices = (index: number) => {
		const _unselectedChoices = unselectedChoices.filter((x) => x !== index);
		const _selectedChoices = [...selectedChoices];
		_selectedChoices.push(index);
		setunSelectedChoices(_unselectedChoices);
		setSelectedChoices(_selectedChoices);
	};

	const removeFromSelectedChoices = (index: number) => {
		const _selectedChoices = selectedChoices.filter((x) => x !== index);
		const _unselelectedChoices = [...unselectedChoices];
		_unselelectedChoices.push(index);
		setSelectedChoices(_selectedChoices);
		setunSelectedChoices(
			_unselelectedChoices.sort((a, b) => {
				return a - b;
			})
		);
	};
	...
	const castVote = async () => {
		if (!library || !account || isEmpty(selectedChoices) || score <= 0) {
			return;
		}
		try {
			const receipt = await client.vote(library, account, {
				...
				type: proposalTypes.APPROVAL,
				choice: selectedChoices.map((x) => x + 1),
	...
	useEffect(() => {
		...
		setunSelectedChoices(proposal.choices.map((_choice, index) => index));
	}, [proposal]);
				...
				<Grid container className='grid-row cast-vote'>
					<Grid className='row-item'>
						...
						>
								...
								<div className='selected-choices'>
									{selectedChoices.map(
										(choiceIndex, index) => {
											return (
												<div
													className='choice text-center selected d-flex justify-content-between'
													key={choiceIndex}
													onClick={() => {
							...
							<div className='unselected-choices'>
								{unselectedChoices.map((choiceIndex) => {
									return (
										<div
											className='choice text-center d-flex justify-content-between'
											key={choiceIndex}
											onClick={() => {
							...
							<div className='card-body'>
								{page === 1 && account && voteWithScores && (
									...
									>
										...
										<Grid item sm={4}>
											{isArray(voteWithScores.choice) && (
												<Tooltip
													title={voteWithScores.choice
														.map((x) => {
															return proposal
																.choices[x - 1];
														})
...
const ResultBox = ({
	...
	votesWithScores.forEach((_voteWithScore, index) => {
		if (isArray(_voteWithScore.choice) && _voteWithScore.choice.length) {
			choiceScoresObj[_voteWithScore.choice[0]] = {
				...choiceScoresObj[_voteWithScore.choice[0]],
				score:
					choiceScoresObj[_voteWithScore.choice[0]].score +
					totalStrategyVotes(_voteWithScore.vp_by_strategy),
			};
		}
	});
