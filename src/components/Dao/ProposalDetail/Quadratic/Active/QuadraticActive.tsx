...
const SingleChoiceResultBox = ({
	...
	let voterChoiceCalculatedVps = [] as Array<number>;
	voterChoiceCalculatedVps = voterChoiceVPs.map((voterVoiceVP) => {
		const temp = voterVoiceVP.reduce((previousValue, currentValue) => {
			return previousValue + Math.sqrt(currentValue);
		}, 0);
		return temp ** 2;
	});

	const totalCalculatedVP = voterChoiceCalculatedVps.reduce(
		(previousValue, currentValue) => {
			return previousValue + currentValue;
		},
		0
	...
	proposal.choices.forEach((_choice, index) => {
		const id = index + 1;
		const percentage = voterChoiceCalculatedVps[index]
			? voterChoiceCalculatedVps[index] / totalCalculatedVP
			: 0;
		choiceScoresObj[id] = {
			id,
			choice: _choice,
...
export default QuadraticActive;
