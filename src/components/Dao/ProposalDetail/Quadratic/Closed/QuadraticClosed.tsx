...
const QuadraticClosed = ({ proposal }: { proposal: Proposal }) => {
	...
	useEffect(() => {
		...
		fetchVoteByVoter({ id: proposal.id, voter: account }).then((_vote) => {
			if (_vote && !isArray(_vote.choice) && isObject(_vote.choice)) {
				_vote.totalCasts = Object.values(_vote.choice).reduce(
				...
				_vote.voteString = Object.values(_vote.choice)
					.map((value, index) => {
						if (isUndefined(_vote.totalCasts)) {
							return `0% fo${proposal.choices[index]}`;
						}
						return `${((value / _vote.totalCasts) * 100).toFixed(
							1
						)}% for ${proposal.choices[index]}`;
					})
					.join(', ');
			}
			setVote(_vote);
	...
	useEffect(() => {
		...
		fetchVotes({
			...
			_votes.forEach((_vote) => {
				const voter = _vote.voter;

				if (!isArray(_vote.choice) && isObject(_vote.choice)) {
					_vote.totalCasts = Object.values(_vote.choice).reduce(
						(previousValue, currentValue) => {
							return previousValue + currentValue;
						},
...
export default QuadraticClosed;
